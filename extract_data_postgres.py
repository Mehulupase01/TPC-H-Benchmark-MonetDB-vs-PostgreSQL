...
input_file = 'results1-Postgres.txt'
...
def process_results(file_path):
    ...
    with open(timing_file, 'a') as timing_out:
        for run in runs:
            ...
            table_match = re.search(r'\n(.*?)\n\(\d+ rows\)', run_data, re.DOTALL)
            # print(table_match.groups()[0])
            if table_match:
                ...
                cwd = os.getcwd();
                op_path = os.path.join(cwd,'output')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_file', default='results1-Postgres.txt', help='The input file to process.')
